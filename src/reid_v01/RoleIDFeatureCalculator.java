...
public class RoleIDFeatureCalculator {
 ...
 * 
 * @param args [1] .csv file with u,v,weight records [2] bin fraction 
 * [3] features to calculate filename [4] (out) feature filename [5] (out) id filename
 * @throws IOException
 * @throws ParseException
 */
	public static void main(String[] args) throws IOException, ParseException{
		String graphFile = args[0];
		double binSize = new Double(args[1]).doubleValue();
		String featureNamesFile = args[2];
		...
		curLine = new BufferedReader(new FileReader(featureNamesFile)).readLine();
		String[] fields = curLine.split(",");
		for (String s : fields) featureNames.add(s);
...
//		AttributedGraph graph = null;
		...
		Set<String> reps = calculateReps(graph, candidates, featureNames);
		System.out.println(TimeUtils.dateAsString(System.currentTimeMillis()) + " Done.");
		...
		while(attrs.length > 0 && numIters++ <= MAX_ITERATIONS) {
			...
			System.out.println(TimeUtils.dateAsString(System.currentTimeMillis()) + 
					" Computing representatives.");
			reps = calculateReps(graph, candidates, featureNames);
			System.out.println(TimeUtils.dateAsString(System.currentTimeMillis()) + " Done.");
	...
	private static Set<String> calculateReps(AttributedGraph graph, Set<String> candidates, 
			Set<String> features) {
		Set<String> reps = new HashSet<String>(); 

		for(String attr1 : candidates)
			if(features.contains(attr1))
				reps.add(attr1);
